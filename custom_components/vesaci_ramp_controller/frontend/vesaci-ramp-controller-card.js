class VesaciRampController extends HTMLElement {
  ...
  render() {
    ...
    const quickRows=this._quickDraft.map((q,i)=>`<div class="quick-row quick-time-row"><input data-q="minutes" data-i="${i}" type="number" min="0.1" step="0.1" value="${q.minutes}"><button class="stop" data-del-q="${i}">×</button></div>`).join("");
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .tabs{display:flex;gap:6px;overflow:auto;padding:4px 0 8px}.tab{background:transparent;color:var(--secondary-text-color);white-space:nowrap}.tab.active{background:var(--primary-color);color:var(--text-primary-color)}.page{display:none}.page.active{display:block}.quick-head,.quick-row{display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:center}.quick-time-row{grid-template-columns:1fr auto;max-width:420px}.daily-head,.daily-row{display:grid;grid-template-columns:.7fr .8fr 1fr .7fr 1fr auto;gap:8px;align-items:center}.quick-head,.daily-head{font-size:11px;color:var(--secondary-text-color);margin-bottom:6px}.overview-values{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px}.overview-value{border:1px solid var(--divider-color);border-radius:9px;padding:12px}.overview-value strong{display:block;font-size:20px;margin-top:4px}.log{font-family:monospace;font-size:12px;padding:7px;border-bottom:1px solid var(--divider-color)}
        ...
        <div class="page ${this._tab==="quick"?"active":""}"><div class="section"><div class="section-title">Quick Action time presets</div><div class="hint">Configure the durations shown in the Quick Action card. The target value is entered directly in the card.</div><div class="quick-head quick-time-row"><span>Minutes</span><span></span></div>${quickRows}<div class="actions" style="margin-top:12px"><button id="add_quick">＋ Add time</button><button id="save_quick">Save times</button><span id="quick_message"></span></div></div></div>
...
if (!customElements.get("vesaci-ramp-controller-panel")) customElements.define("vesaci-ramp-controller-panel",VesaciRampController);
if (!customElements.get("vesaci-ramp-controller-card")) customElements.define("vesaci-ramp-controller-card",class extends VesaciRampController{});
...
class VesaciRampQuickActionsCard extends HTMLElement {
  ...
  esc(value){return String(value??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]));}
  render(){if(!this.shadowRoot||!this._hass)return;const state=this.current();if(!state){this.shadowRoot.innerHTML=`<ha-card><div style="padding:16px">No Ramp Controller found.</div></ha-card>`;return;}const actions=state.attributes.quick_actions||[],minutes=[...new Set(actions.map(q=>Number(q.minutes)).filter(Number.isFinite))].sort((a,b)=>a-b),targetState=this._hass.states[state.attributes.target_entity],minimum=targetState?.attributes?.min,maximum=targetState?.attributes?.max,step=targetState?.attributes?.step??"any",defaultTarget=Number(targetState?.state??state.attributes.current_value??0);this._minutes=minutes.includes(Number(this._minutes))?Number(this._minutes):(minutes[0]??5);if(this._target===null||!Number.isFinite(Number(this._target)))this._target=Number.isFinite(defaultTarget)?defaultTarget:0;this.shadowRoot.innerHTML=`<style>ha-card{padding:16px}.title{font-size:18px;font-weight:650;margin-bottom:12px}.form{display:grid;grid-template-columns:1fr 1fr auto;gap:10px;align-items:end}label{display:flex;flex-direction:column;gap:5px;font-size:11px;color:var(--secondary-text-color)}select,input{box-sizing:border-box;width:100%;background:var(--card-background-color);color:var(--primary-text-color);border:1px solid var(--divider-color);border-radius:8px;padding:10px}button{border:0;border-radius:8px;padding:11px 20px;background:var(--primary-color);color:var(--text-primary-color);font-weight:650}@media(max-width:500px){.form{grid-template-columns:1fr 1fr}.form button{grid-column:span 2}}</style><ha-card><div class="title">Quick Action</div><div class="form"><label>Time preset<select id="quick_minutes">${minutes.map(value=>`<option value="${value}" ${value===this._minutes?"selected":""}>${value} min</option>`).join("")}</select></label><label>Target value<input id="quick_target" type="number" required step="${this.esc(step)}" ${minimum!==undefined?`min="${this.esc(minimum)}"`:""} ${maximum!==undefined?`max="${this.esc(maximum)}"`:""} value="${this.esc(this._target)}"></label><button id="quick_start">▶ Start</button></div></ha-card>`;const minutesSelect=this.shadowRoot.getElementById("quick_minutes"),targetInput=this.shadowRoot.getElementById("quick_target");minutesSelect.onchange=()=>{this._minutes=Number(minutesSelect.value);};targetInput.oninput=()=>{this._target=targetInput.value===""?NaN:Number(targetInput.value);};this.shadowRoot.getElementById("quick_start").onclick=()=>{if(targetInput.reportValidity()&&Number.isFinite(this._target))this.run();};}
}
if (!customElements.get("vesaci-ramp-controller-quick-actions-card")) customElements.define("vesaci-ramp-controller-quick-actions-card",VesaciRampQuickActionsCard);
window.customCards=window.customCards||[];
const vesaciCards=[
  {type:"vesaci-ramp-controller-card",name:"Vesaci Ramp Controller",description:"Visual ramp profile editor and controller"},
  {type:"vesaci-ramp-controller-compact-card",name:"Vesaci Ramp Controller Compact",description:"Compact controls, current value, action and progress"},
  {type:"vesaci-ramp-controller-quick-actions-card",name:"Vesaci Ramp Quick Action",description:"Select duration and target, then start"},
];
vesaciCards.forEach(card=>{if(!window.customCards.some(item=>item.type===card.type))window.customCards.push(card);});
