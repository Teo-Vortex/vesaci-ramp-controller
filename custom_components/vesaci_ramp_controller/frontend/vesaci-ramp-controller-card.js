class VesaciRampController extends HTMLElement {
  ...
  render() {
    ...
    const quickRows=this._quickDraft.map((q,i)=>`<div class="quick-row"><input data-q="name" data-i="${i}" value="${this.esc(q.name)}"><input data-q="minutes" data-i="${i}" type="number" min="0.1" value="${q.minutes}"><input data-q="target" data-i="${i}" type="number" value="${q.target}"><select data-q="curve" data-i="${i}">${options(["linear","ease_in","ease_out","s_curve"],q.curve||"linear")}</select><button data-run-q="${q.id}">Run</button><button class="stop" data-del-q="${i}">×</button></div>`).join("");
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        <div class="page ${this._tab==="quick"?"active":""}"><div class="section"><div class="section-title">Quick Actions</div><div class="quick-head"><span>Name</span><span>Minutes</span><span>Target</span><span>Curve</span><span></span><span></span></div>${quickRows}<div class="actions" style="margin-top:12px"><button id="add_quick">＋ Add action</button><button id="save_quick">Save Quick Actions</button><span id="quick_message"></span></div></div></div>
  ...
  bind(profile, controlPoints) {
    ...
    this.shadowRoot.querySelectorAll("[data-run-q]").forEach(button=>button.onclick=()=>this.call("execute_quick",{action_id:button.dataset.runQ}));
    ...
    $("add_quick").onclick=()=>{this._quickDraft.push({id:`quick_${Date.now()}`,name:"New action",minutes:10,target:profile.upper_target,curve:"linear"});this._editing=true;this.render();};
    $("save_quick").onclick=async()=>{const message=$("quick_message");if(this._quickDraft.some(q=>!q.name||q.minutes<=0)){message.textContent="Name and positive minutes are required.";return;}message.textContent="Saving…";await this.call("save_quick_actions",{actions:JSON.stringify(this._quickDraft)});this._editing=false;message.textContent="Saved";};
...
class VesaciRampQuickActionsCard extends HTMLElement {
  ...
  run(id){const state=this.current();if(state)this._hass.callService("vesaci_ramp_controller","execute_quick",{controller_id:state.attributes.controller_id,action_id:id});}
  esc(value){return String(value??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]));}
  render(){if(!this.shadowRoot||!this._hass)return;const state=this.current();if(!state){this.shadowRoot.innerHTML=`<ha-card><div style="padding:16px">No Ramp Controller found.</div></ha-card>`;return;}const actions=state.attributes.quick_actions||[],columns=Math.max(1,actions.length);this.shadowRoot.innerHTML=`<style>ha-card{padding:16px}.title{font-size:18px;font-weight:650;margin-bottom:12px}.actions{display:grid;grid-template-columns:repeat(${columns},minmax(70px,1fr));gap:8px;overflow:auto}.cell{display:grid;grid-template-rows:1fr 1fr;gap:6px}.cell button{border:0;border-radius:8px;padding:10px 5px;background:var(--primary-color);color:var(--text-primary-color);font-weight:600}.cell button:last-child{background:#59636e}.name{text-align:center;font-size:11px;color:var(--secondary-text-color);margin-bottom:3px}</style><ha-card><div class="title">Quick Actions</div><div class="actions">${actions.map(q=>`<div><div class="name">${this.esc(q.name)}</div><div class="cell"><button data-q="${q.id}">${q.minutes} min</button><button data-q="${q.id}">${this.esc(q.target)}</button></div></div>`).join("")}</div></ha-card>`;this.shadowRoot.querySelectorAll("[data-q]").forEach(button=>button.onclick=()=>this.run(button.dataset.q));}
}
...
window.customCards.push({type:"vesaci-ramp-controller-quick-actions-card",name:"Vesaci Ramp Quick Actions",description:"Two-row quick ramp actions"});
