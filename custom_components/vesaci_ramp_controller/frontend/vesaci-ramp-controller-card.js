...
class VesaciRampCompactCard extends HTMLElement {
  ...
  esc(value){return String(value??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]));}
  render(){if(!this.shadowRoot||!this._hass)return;const all=this.controllers(),item=this.current();if(!item){this.shadowRoot.innerHTML=`<ha-card><div style="padding:20px">No Ramp Controller found.</div></ha-card>`;return;}const a=item.state.attributes,profiles=a.profiles||[],selected=this._profileId||a.selected_profile||profiles[0]?.id,progress=Number(a.progress||0),current=this._hass.states[a.target_entity]?.state??a.current_value??"—",action=`${(a.action||"—").toUpperCase()} · ${String(item.state.state).toUpperCase()}`;this.shadowRoot.innerHTML=`<style>:host{display:block}ha-card{padding:16px}.head,.values,.buttons{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.head{justify-content:space-between}.title{font-size:18px;font-weight:650}.values{margin:14px 0}.value{flex:1;min-width:100px;padding:10px;border:1px solid var(--divider-color);border-radius:9px}.label{font-size:11px;color:var(--secondary-text-color)}.big{font-size:20px;margin-top:3px}.bar{height:9px;background:var(--divider-color);border-radius:9px;overflow:hidden;margin:10px 0 14px}.fill{height:100%;width:${Math.max(0,Math.min(100,progress))}%;background:var(--primary-color)}select{background:var(--card-background-color);color:var(--primary-text-color);border:1px solid var(--divider-color);border-radius:8px;padding:9px}button{flex:1;border:0;border-radius:8px;padding:10px;background:var(--primary-color);color:var(--text-primary-color);font-weight:600}button.down{background:#59636e}button.stop{background:var(--error-color)}</style><ha-card><div class="head"><div class="title">Ramp Controller</div>${all.length>1?`<select id="controller">${all.map(x=>`<option value="${x.state.attributes.controller_id}" ${x===item?"selected":""}>${this.esc(x.state.attributes.target_entity)}</option>`).join("")}</select>`:""}</div><select id="mode" style="margin-top:12px">${profiles.map(p=>`<option value="${p.id}" ${p.id===selected?"selected":""}>${this.esc(p.name)}</option>`).join("")}</select><div class="values"><div class="value"><div class="label">Current value</div><div class="big">${this.esc(current)}</div></div><div class="value"><div class="label">Action</div><div class="big">${this.esc(action)}</div></div><div class="value"><div class="label">Progress</div><div class="big">${progress.toFixed(1)}%</div></div></div><div class="bar"><div class="fill"></div></div><div class="buttons"><button id="up">▲ UP</button><button class="down" id="down">▼ DOWN</button><button id="pause">Ⅱ</button><button id="resume">▶</button><button class="stop" id="stop">■</button></div></ha-card>`;const $=id=>this.shadowRoot.getElementById(id);if($("controller"))$("controller").onchange=e=>{this._controller=e.target.value;this._profileId=null;this.render();};$("mode").onchange=e=>{this._profileId=e.target.value;this.call("select_profile",{profile:e.target.value});};$("up").onclick=()=>this.call("start_profile",{profile:$("mode").value,direction:"up"});$("down").onclick=()=>this.call("start_profile",{profile:$("mode").value,direction:"down"});$("pause").onclick=()=>this.call("pause");$("resume").onclick=()=>this.call("resume");$("stop").onclick=()=>this.call("stop");}
}
